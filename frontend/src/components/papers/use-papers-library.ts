...
import {
  listPapersQueryOptions,
  papersSearchToParams,
  toPapersSearch,
} from '#/lib/papers.ts'

const papersRoute = getRouteApi('/dashboard/papers/')

export function usePapersLibrary() {
  const search = papersRoute.useSearch()
  const navigate = papersRoute.useNavigate()
  ...
  const params = papersSearchToParams(search)
  const papersQuery = useQuery(
    listPapersQueryOptions(params, getToken, isLoaded),
  )
  ...
  function commitSearch(query: string, offset?: number) {
    ...
    void navigate({
      search: toPapersSearch(query, offset),
      replace: true,
    })
  }
  ...
  return {
    ...
    errorMessage:
      papersQuery.error instanceof Error
        ...
        : 'The papers request failed. Try again.',
    applySearch,
    onSearchDraftChange,
    goNext,
    goPrev,
