...
import { ingestStatusLabel, toOpenAlexWorkId, type PaperResponse } from '#/lib/papers.ts'

const papersRoute = getRouteApi('/dashboard/papers/')

export function PaperResult({ paper }: { paper: PaperResponse }) {
  const papersSearch = papersRoute.useSearch()
    ...
    <article className="flex flex-col gap-2 px-4 py-5 sm:px-6">
      <div className="flex flex-wrap items-start justify-between gap-2">
        ...
        <div className="flex flex-wrap items-center gap-1.5">
          {paper.already_added ? (
            <Badge variant="secondary">On your list</Badge>
          ) : null}
          <Badge
